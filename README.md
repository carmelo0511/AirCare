...
8. Serve the frontend locally from the project root. Make sure you've run
   `scripts/set-api-url.js` (step 6) so that `frontend/config.js` exists:
   ```bash
...
- **404 for `config.js`** – this file is generated by step 6. Run
  `API_BASE_URL=https://your-api-endpoint node scripts/set-api-url.js` to
  create `frontend/config.js` before launching the server.
